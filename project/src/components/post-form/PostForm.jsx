...
export default function PostForm({ post }) {
    ...
    // Debugging: Log userData to verify its value
    console.log("PostForm - userData:", userData);

    // Ensure hooks are always called in the same order
    useEffect(() => {
        if (userData?.$id) { // Use $id instead of userId (Appwrite uses $id)
            setIsLoading(false);
        } else {
            navigate('/login');
        }
    ...
    const submit = async (data) => {
        if (!userData?.$id) { // Use $id instead of userId
            navigate('/login');
            return;
        }
    ...
    return (
        <form onSubmit={handleSubmit(submit)} className="flex flex-wrap">
            ...
            <div className="w-1/3 px-2">
                ...
                <Button
                    type="submit"
                    bgColor={post ? "bg-green-500 hover:bg-green-600 cursor-pointer" : "bg-blue-500 hover:bg-blue-600 curser-pointer"}
